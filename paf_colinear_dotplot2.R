...
input_filename_without_ext <- tools::file_path_sans_ext(basename(opt$input_filename))
...
total_query_length <- sum(unique(alignments$queryLen))
total_ref_length <- sum(unique(alignments$refLen))
total_genome_length <- total_query_length + total_ref_length

# Identify aligned regions and calculate aligned length
aligned_length <- sum(alignments$refEnd - alignments$refStart)

# Calculate percent collinearity
percent_collinearity <- (aligned_length / total_genome_length) * 100

cat(paste0("Total Genome Length: ", total_genome_length, "\n"))
cat(paste0("Aligned Length: ", aligned_length, "\n"))
cat(paste0("Percent Collinearity: ", percent_collinearity, "%\n"))

# gp
ggsave(filename = paste0(input_filename_without_ext, "_dotplot.png"), width = opt$plot_size, height = opt$plot_size, units = "in", dpi = 300, limitsize = F)
